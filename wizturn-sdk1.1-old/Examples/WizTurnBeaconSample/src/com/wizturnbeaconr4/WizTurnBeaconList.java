...
import com.wizturn.sdk.connect.WizTurnBeaconConnect.WriteCallback;
import com.wizturn.sdk.entity.WizTurnBeacons;
import com.wizturnbeaconr4.adapter.WizTurnBeaconListAdapter;
import com.wizturnbeaconr4.popup.IPopupAuthenticate;
import com.wizturnbeaconr4.popup.PopupManager;
...
public class WizTurnBeaconList extends Activity implements OnClickListener , OnItemClickListener{
	...
	private void authenticate() {
		if(_connect.isCanAuthenticate()) {
			runOnUiThread(new Runnable() {
				@Override
				public void run() {
					_connect.setAuthenticateNotification(new WriteCallback() {
						...
						public void onError() {
							...
							mBtn_Connect.setVisibility(View.VISIBLE);
							_connect.close();
						}
						
						@Override
						public void onDescriptorWrite() {
							PopupManager.getInstance().showAuthenticate(mContext, new IPopupAuthenticate() {

								@Override
								public void onDismiss(DialogInterface dialog,
										String password) {
									showDialog("Authenticating...");
									_connect.tryAuthenticate(password, new WriteCallback() {
